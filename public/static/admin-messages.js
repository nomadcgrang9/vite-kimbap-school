...
// ============ GLOBAL VARIABLES ============
let currentStudents = []; // 현재 로드된 학생 목록
...
async function loadStudentsList() {
    ...
    try {
        ...
        if (currentStudents.length > 0) {
            // 학급 정보도 확인  
            const sampleStudent = currentStudents[0];
        ...
        currentStudents = JSON.parse(localStudents || '[]');
        console.log(`[Messages] 폴백 후 학생 수: ${currentStudents.length}`);
    }
...
async function sendClassMessage(targetClass, message) {
    const classStudents = currentStudents.filter(student => {
...
async function sendAllStudentsMessage(message) {
    ...
    return { successCount, failCount, totalCount: currentStudents.length };
}
...
function updateStudentSelectionUI() {
    console.log(`[Messages] UI 업데이트 시작 - 학생 수: ${currentStudents.length}`);
    ...
    if (classSelect) {
        // 학급 목록 추출 (중복 제거)
        const classes = [...new Set(currentStudents.map(student => {
    ...
    if (totalStudentsCount) {
        totalStudentsCount.textContent = currentStudents.length;
        console.log(`[Messages] 전체 학생 수 업데이트: ${currentStudents.length}명`);
    } else {
...
async function handleAllStudentsMessageSend() {
    ...
    // 확인 대화상자
    if (!confirm(`전체 학생 ${currentStudents.length}명에게 메시지를 전송하시겠습니까?`)) {
        return;
    }
