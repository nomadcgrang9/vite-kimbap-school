// Cloudflare Pages Functions - Tables API 엔드포인트
// Supabase 설정
...
export async function onRequestGet(context: any) {
  ...
  const tableName = pathParts[2] // /tables/assignments -> 'assignments'
  
  try {
    // URL 쿼리 파라미터 그대로 전달
    const queryParams = url.search
    const supabaseUrl = `${SUPABASE_URL}/rest/v1/${tableName}${queryParams}`
    ...
    console.error(`Tables API error for ${tableName}:`, error)
...
export async function onRequestPost(context: any) {
  ...
  try {
    ...
    const supabaseUrl = `${SUPABASE_URL}/rest/v1/${tableName}`
    ...
    console.error(`POST Tables API error for ${tableName}:`, error)
    return addCorsHeaders(new Response(JSON.stringify({
      error: error instanceof Error ? error.message : 'Unknown error'
    }), {
      status: 500,
